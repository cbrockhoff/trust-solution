...
import (
	...
	"github.com/cbrockhoff/user/db"
	"github.com/cbrockhoff/user/users"
...
type Service interface {
	Login(username, password string) (users.User, string, error) // GET /login
	Register(username, password, email, first, last string) (string, string, error)
...
func (s *fixedService) Login(username, password string) (users.User, string, error) {
	u, err := db.GetUserByName(username)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return users.New(), "no_token", err
	}
	if u.Password != calculatePassHash(password, u.Salt) {
		fmt.Fprintln(os.Stderr, err)
		return users.New(), "no_token", ErrUnauthorized
	}
	db.GetUserAttributes(&u)
	u.MaskCCs()
	t := createToken(u.UserID)
	return u, t, nil
}

func (s *fixedService) Register(username, password, email, first, last string) (string, string, error) {
	u := users.New()
	u.Username = username
