...
import works.weave.socks.orders.services.TokenGetService;
...
public class OrdersController {
    ...
    public
    @ResponseBody
    CustomerOrder newOrder(@RequestBody NewOrderResource item, @RequestHeader(value="User-Token", defaultValue="") String userToken) {
        try {
            ...
            LOG.info("Received call from user with token: " + userToken);
            String serviceToken = tokenGetService.getServiceToken();
            LOG.info("My service token: " + serviceToken);

            LOG.debug("Starting calls");
            Future<Resource<Address>> addressFuture = asyncGetService.getResource(item.address, userToken, serviceToken, new TypeReferences
                    .ResourceType<Address>() {
            });
            Future<Resource<Customer>> customerFuture = asyncGetService.getResource(item.customer, userToken, serviceToken, new TypeReferences
                    .ResourceType<Customer>() {
            });
            Future<Resource<Card>> cardFuture = asyncGetService.getResource(item.card, userToken, serviceToken, new TypeReferences
                    .ResourceType<Card>() {
            });
            Future<List<Item>> itemsFuture = asyncGetService.getDataList(item.items, userToken, serviceToken, new
            ...
            Future<Shipment> shipmentFuture = asyncGetService.postResource(
